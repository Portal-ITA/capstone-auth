# A Simple node Library that Authenticates a User Against an LDAP/AD Server forked from https://github.com/shaozi/ldap-authentication


## Goal

Make authentication with an LDAP server easy customized to CapstoneJS
...
```sh
npm install capstone-auth --save
```
...
```javascript
const { authenticate } = require('capstone-auth')
